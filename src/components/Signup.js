...
export default function Signup({ onSignup, switchToLogin }) {
  ...
  async function submit(e) {
    ...
    setLoading(true);
    try {
      // Only members can be created via signup form
      const data = await signup({ username: username.trim(), password, role: 'member' });
      onSignup(data.user);
    } catch (err) {
      if (err.networkError) {
          ...
          <form onSubmit={submit} className="auth-form">
            <div className="mb-4">
              ...
              <input 
                ...
                disabled={loading}
              />
              <small className="form-text text-muted mt-1">This will be your unique identifier</small>
            </div>
            ...
            {/* admin account note removed per user request */}
            ...
            <div className="text-center">
              <p className="auth-switch-text mb-0">
                ...
                <button type="button" className="btn-link" onClick={switchToLogin} disabled={loading}>
                  Sign in
                </button>
              </p>
            </div>
